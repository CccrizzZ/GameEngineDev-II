...
void A1::BuildMaterials()
{
	...
	mMaterials["BackgroundMat"] = std::move(BackgroundMat);
...
{
	Background = std::make_unique<RenderItem>();
	XMStoreFloat4x4(&Background->World, XMMatrixScaling(2.5f, 1.0f, 2.5f) * XMMatrixTranslation(0.0f, 0.0f, 0.0f));
	Background->ObjCBIndex = 0;
	Background->Mat = mMaterials["BackgroundMat"].get();
	Background->Geo = mGeometries["shapeGeo"].get();
	//landRitem->World = MathHelper::Identity4x4();
	Background->PrimitiveType = D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
	Background->IndexCount = Background->Geo->DrawArgs["Quad"].IndexCount;
	Background->StartIndexLocation = Background->Geo->DrawArgs["Quad"].StartIndexLocation;
	Background->BaseVertexLocation = Background->Geo->DrawArgs["Quad"].BaseVertexLocation;
	mRitemLayer[(int)RenderLayer::Opaque].push_back(Background.get());
	mAllRitems.push_back(std::move(Background));

	Plane = std::make_unique<RenderItem>();
	XMStoreFloat4x4(&Plane->World, XMMatrixScaling(0.1f, 1.0f, 0.1f) * XMMatrixTranslation(0.0f, 0.1f, -10.0f));
	Plane->ObjCBIndex = 1;
	Plane->Mat = mMaterials["PlaneMat"].get();
	Plane->Geo = mGeometries["shapeGeo"].get();
	Plane->PrimitiveType = D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
	Plane->IndexCount = Plane->Geo->DrawArgs["Quad"].IndexCount;
	Plane->StartIndexLocation = Plane->Geo->DrawArgs["Quad"].StartIndexLocation;
	Plane->BaseVertexLocation = Plane->Geo->DrawArgs["Quad"].BaseVertexLocation;
	mRitemLayer[(int)RenderLayer::Transparent].push_back(Plane.get());
	mAllRitems.push_back(std::move(Plane));
